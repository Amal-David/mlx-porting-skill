...
| The deep research loop emits review-only assignments, dynamic assignment-planner receipts, subagent handoff manifests, campaign-wave receipts, campaign-runner receipts, explicit sampling-target receipts, externally ingested result receipts, planned-versus-sampled coverage, review-readiness gates, per-iteration and loop-level promotion-review ledgers, fixed and adaptive iterative loop receipts, planned multi-source sampling, worker-authored or generated blogs with provenance, synthesis, bounded parallel executor receipts, and rejects malformed findings | `test_research_loop_*`, `test_research_campaign_runner_*` |
