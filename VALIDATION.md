...
| Offline tests | 447 | `python3 -m unittest discover -s tests` |
...
| Dense-decoder and non-gated ReLU T5 source capture, fail-closed scaffold generation, schema-2 conversion, MLX capture, cache checks, and first-divergence parity have dependency-free contracts plus gated Torch/MLX execution tests. | `tests/test_capture_oracle_contract.py`, `tests/test_scaffold_port_contract.py`, `tests/test_encoder_decoder_contract.py`, `tests/test_convert_checkpoint_contract.py`, `tests/test_parity_runner_contract.py` |
