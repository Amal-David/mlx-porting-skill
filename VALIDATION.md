...
| The deep research loop emits review-only assignments, dynamic assignment-planner receipts, subagent handoff manifests, externally ingested result receipts, planned-versus-sampled coverage, review-readiness gates, fixed and adaptive iterative loop receipts, planned multi-source sampling, worker-authored or generated blogs with provenance, synthesis, bounded parallel executor receipts, and rejects malformed findings | `test_research_loop_*` |
...
- Live execution of external research subagents or network collectors; offline tests cover the harness contract, bounded local executor concurrency, iterative receipts, fixture ingestion, and contributor collector receipt behavior only.
