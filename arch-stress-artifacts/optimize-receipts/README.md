...
with the quality gate enforcing the perplexity ratio and degenerate-output rate. First-token
agreement is recorded per config, but these sweeps set its minimum to 0.0, so it did not gate.
...
| Qwen2.5-1.5B | 32.8 tok/s — **quality FAIL** | 22.0 tok/s — **quality PASS** | 8-bit chosen (held quality) |
...
**Consistent finding:** the naive default (4-bit-g64) is consistently smaller and usually faster, but
**fails the quality gate** on these small models; the structured pick is 8-bit, the highest
throughput-per-peak-memory config that *held quality*. The gate earns its keep — it stops you
shipping a faster port that quietly degraded.
